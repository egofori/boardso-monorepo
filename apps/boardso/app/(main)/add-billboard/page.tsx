...
export default function Page() {
  ...
  const menu = (
    <UIMenuList className="max-h-[500px] max-w-[100px] rounded-xl">
      {unitsOfMeasurement.map((data: any) => (
        <UIMenuItem
          key={data.value}
          onClick={() => setSelectedUnitOfMeasurement(data)}>
    ...
    <main className="layout-wrapper">
      <UICard className="mx-auto my-5 max-w-3xl p-10 bg-white">
        ...
        <UIForm form={form} className="flex flex-col gap-6 mt-8">
          ...
          <div>
            <label>Type</label>
            <UISelect placeholder="Select billboard type" value={selectedBillboardType.value} onChange={(value) => console.log("value", value)}>
              {billboardTypes.map((data) => (
                <UIOption key={data.value} value={data.value} onClick={() => setSelectedBillboardType(data)}>
                  {data.label}
                </UIOption>
              ))}
            </UISelect>
            ...
            <div className="flex items-center gap-2">
              <div className="flex flex-col w-full">
                <div className="flex">
                  ...
                  <UIDropdownButton
                    menu={menu}
                    className="border border-l-0 border-blue-gray-200 rounded-l-none bg-slate-100">
                    <UITypography className="font-normal normal-case">
                      {selectedUnitOfMeasurement.label}
                    </UITypography>
                  </UIDropdownButton>
                </div>
                <div className="h-4"><UIFieldError name="width" /></div>
              </div>
              ...
              <div className="flex flex-col w-full">
                ...
                <div className="h-4"><UIFieldError name="height" /></div>
              </div>
            </div>
          </div>
          <div>
